...
const STATUS_CONFIG = {
  ...
  no_tests:   { label: 'No tests',   color: 'bg-slate-100 text-slate-500', dot: 'bg-slate-300' },
}
...
export function CoveragePanel({ projectId }: { projectId: string }) {
  ...
  if (!data || data.matrix.length === 0) return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="text-sm text-slate-500">No coverage data yet.</p>
      <p className="mt-1 text-xs text-slate-400">Extract requirements and run tests to see coverage.</p>
    </div>
  )
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        {[
          ...
          { label: 'No tests', value: summary.noTests,  color: 'text-slate-400' },
      ...
      <div className="rounded-xl border border-slate-200 bg-white px-5 py-4">
        <div className="mb-2 flex items-center justify-between">
          <p className="text-xs text-slate-500">{summary.covered} of {summary.total} requirements covered</p>
      ...
      {trends && trends.points.length > 1 && (
        <div className="rounded-xl border border-slate-200 bg-white px-5 py-4">
          <p className="mb-3 text-xs font-medium uppercase tracking-wider text-slate-400">Pass Rate Trend</p>
              ...
              <div className="divide-y divide-red-100 rounded-xl border border-red-200 bg-red-50/30">
                {regressionData.regressions.map((r) => (
                  <div key={r.testId} className="flex items-center gap-2 px-4 py-2.5">
                    ...
                    <p className="text-xs text-slate-700">{r.testName}</p>
              ...
              <div className="divide-y divide-amber-100 rounded-xl border border-amber-200 bg-amber-50/30">
                {regressionData.flaky.map((f) => (
                  <div key={f.testId} className="flex items-center gap-2 px-4 py-2.5">
                    ...
                    <p className="text-xs text-slate-700">{f.testName}</p>
            ...
            {gaps.gaps.map((gap) => (
              <div key={gap.id} className="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-orange-50">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm text-slate-800">{gap.title}</p>
                  {gap.description && <p className="truncate text-xs text-slate-400">{gap.description}</p>}
                </div>
                <span className={`shrink-0 text-xs font-medium ${PRIORITY_COLOR[gap.priority] ?? 'text-slate-400'}`}>
                  {gap.priority}
                </span>
              </div>
            ))}
      ...
      {/* Requirements Matrix */}
      <div>
        <p className="mb-2 text-xs font-medium uppercase tracking-wider text-slate-400">Requirements Matrix</p>
        <div className="divide-y divide-slate-100 rounded-xl border border-slate-200 bg-white">
          {data.matrix.map(({ requirement: req, status, testCount, passedCount }) => {
            const cfg = STATUS_CONFIG[status]
            return (
              <div key={req.id} className="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-slate-50">
                <span className={`mt-0.5 h-2 w-2 shrink-0 rounded-full ${cfg.dot}`} />
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm text-slate-800">{req.title}</p>
                  <p className="text-xs text-slate-400">
                    {req.module || req.type || 'functional'}
                    {testCount > 0 && ` · ${passedCount}/${testCount} tests passing`}
                  </p>
                </div>
                <span className={`shrink-0 text-xs font-medium ${PRIORITY_COLOR[req.priority] ?? 'text-slate-400'}`}>
                  {req.priority}
                </span>
