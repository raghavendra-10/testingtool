      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        <div className="mb-4 flex gap-1 rounded-lg bg-slate-100 p-1 w-fit">
          <button
            ...
            className={`rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${tab === 'code' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Code Analysis
          </button>
          <button
            onClick={() => { setTab('schema'); setSeverityFilter('all'); setCategoryFilter('all') }}
            className={`rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${tab === 'schema' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Schema Analysis
          </button>
        </div>
        ...
        {tab === 'code' && (
          <>
            <h3 className="mb-1 text-sm font-medium text-slate-700">Analyze Source Code</h3>
            ...
            {codeRuns && codeRuns.length > 0 && (
              <div className="mt-4">
                <p className="mb-2 text-xs font-medium uppercase tracking-wider text-slate-400">Analysis Runs</p>
            ...
            {codeRunDetail && codeRunDetail.issues.length > 0 && (
              <IssueList
                ...
                renderIssue={(issue: CodeIssue) => (
                  <div key={issue.id} className="border-b border-slate-100 px-4 py-3 last:border-b-0">
                    <div className="flex items-start gap-2">
                      <span className={`mt-0.5 h-2 w-2 shrink-0 rounded-full ${SEVERITY_DOT[issue.severity] ?? 'bg-slate-300'}`} />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {issue.ruleId && (
                            <span className="text-[10px] text-slate-400 font-mono">{issue.ruleId}</span>
                          )}
                        </div>
                        ...
                        {issue.codeSnippet && (
                          <pre className="mt-2 rounded bg-slate-950 p-2 text-xs text-slate-300 font-mono overflow-x-auto max-h-24">
                            {issue.codeSnippet}
                          </pre>
                        )}
        ...
        {tab === 'schema' && (
          <>
            <h3 className="mb-1 text-sm font-medium text-slate-700">Analyze Database Schema</h3>
            <p className="mb-3 text-xs text-slate-400">
            ...
            <div className="mb-3">
              <textarea
                ...
                rows={8}
                className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:border-indigo-400 focus:outline-none font-mono"
              />
            </div>
            <button
            ...
            {schemaRuns && schemaRuns.length > 0 && (
              <div className="mt-4">
                <p className="mb-2 text-xs font-medium uppercase tracking-wider text-slate-400">Schema Runs</p>
                <div className="divide-y divide-slate-100 rounded-lg border border-slate-200">
                  {schemaRuns.map((run) => (
                    <div
                      key={run.id}
...
function IssueList<T extends { severity: string; category: string }>({
  ...
  return (
    <div className="mt-4">
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <select
            ...
            className="rounded border border-slate-200 px-2 py-1 text-xs text-slate-600 focus:border-indigo-400 focus:outline-none"
          >
      ...
      {/* Issues */}
      <div className="rounded-xl border border-slate-200 bg-white overflow-hidden max-h-[65vh] overflow-y-auto">
        {issues.length === 0 ? (
