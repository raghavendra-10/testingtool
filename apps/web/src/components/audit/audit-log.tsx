          ...
          {data.logs.map(entry => (
            <div key={entry.id} className="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-muted/50">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm text-foreground">{entry.resourceType}{entry.resourceId ? ` #${entry.resourceId.slice(0, 8)}` : ''}</p>
                <p className="text-xs text-muted-foreground">by {entry.userId.slice(0, 12)}...{entry.ipAddress ? ` from ${entry.ipAddress}` : ''}</p>
              </div>
              <span className="shrink-0 text-xs text-muted-foreground" title={new Date(entry.createdAt).toLocaleString()}>
