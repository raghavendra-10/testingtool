...
export function SetupWizard() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-xl font-bold text-slate-900">Set Up Your Project</h1>
        <p className="mt-1 text-sm text-slate-500">Configure everything Speclyn needs to test your API</p>
      </div>
      ...
      <div className="mb-8 flex items-center gap-1">
        {STEPS.map((s, i) => (
          <div key={s.num} className="flex items-center flex-1">
            ...
            {i < STEPS.length - 1 && <div className="h-px w-2 bg-slate-200 shrink-0" />}
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6">
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-200 px-4 py-3 text-sm text-slate-800 placeholder:text-slate-400 focus:border-indigo-400 focus:outline-none"
            />
            <textarea
              value={projectDesc} onChange={e => setProjectDesc(e.target.value)}
              placeholder="Description (optional)"
              rows={3}
              className="w-full rounded-lg border border-slate-200 px-4 py-3 text-sm text-slate-800 placeholder:text-slate-400 focus:border-indigo-400 focus:outline-none resize-none"
            />
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-4">
            <h2 className="text-sm font-semibold text-slate-800">Spec Documents</h2>
            <p className="text-xs text-slate-400">Upload files, paste URLs, or enter text. Add as many as you need.</p>
              ...
              <div className="divide-y divide-slate-100 rounded-xl border border-slate-200">
                {specs.map(s => (
                  <div key={s.id} className="flex items-center gap-3 px-4 py-2.5">
                    ...
                    <p className="flex-1 truncate text-xs text-slate-700">{s.name}</p>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => fileInputRef.current?.click()} className="rounded-lg border border-slate-200 px-4 py-2 text-xs font-medium text-slate-600 hover:bg-slate-50 transition-colors">
                + Upload Files
              </button>
            </div>
            ...
            <div className="flex gap-2">
              <input value={urlInput} onChange={e => setUrlInput(e.target.value)} placeholder="https://petstore.swagger.io/v2/swagger.json"
                className="flex-1 rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700 placeholder:text-slate-400 focus:border-indigo-400 focus:outline-none" />
            ...
            <div className="space-y-2 rounded-xl border border-dashed border-slate-200 p-4">
              <input value={textName} onChange={e => setTextName(e.target.value)} placeholder="Document name"
                className="w-full rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700 placeholder:text-slate-400 focus:border-indigo-400 focus:outline-none" />
              <textarea value={textContent} onChange={e => setTextContent(e.target.value)} placeholder="Paste spec text, requirements, or API documentation..."
                rows={4} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700 placeholder:text-slate-400 focus:border-indigo-400 focus:outline-none resize-y" />
        ...
        {step === 3 && (
          <div className="space-y-4">
            <h2 className="text-sm font-semibold text-slate-800">Connect Repositories</h2>
            <p className="text-xs text-slate-400">Link your code repos so Speclyn can discover API endpoints automatically. Optional — skip if you uploaded specs.</p>
              ...
              <div className="divide-y divide-slate-100 rounded-xl border border-slate-200">
                {repos.map(r => (
                  <div key={r.id} className="flex items-center gap-3 px-4 py-2.5">
                    <span className={`rounded px-1.5 py-0.5 text-[10px] font-bold uppercase ${
                      r.platform === 'github' ? 'bg-slate-900 text-white' :
                      r.platform === 'bitbucket' ? 'bg-blue-600 text-white' :
                      'bg-orange-500 text-white'
                    }`}>{r.platform}</span>
                    <p className="flex-1 truncate text-xs text-slate-700">{r.repoUrl} ({r.branch})</p>
            ...
            <div className="rounded-xl border border-slate-200 p-4 space-y-3">
                <div className="grid gap-3 sm:grid-cols-2">
                  <select value={repoPlatform} onChange={e => setRepoPlatform(e.target.value as typeof repoPlatform)}
                    className="rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700 focus:border-indigo-400 focus:outline-none">
                    <option value="github">GitHub</option>
                    <option value="bitbucket">Bitbucket</option>
                    <option value="gitlab">GitLab</option>
                  </select>
                  <input value={repoBranch} onChange={e => setRepoBranch(e.target.value)} placeholder="Branch (main)"
                    className="rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700 placeholder:text-slate-400 focus:border-indigo-400 focus:outline-none" />
                </div>
        ...
        {step === 4 && (
          <div className="space-y-6">
            <div className="space-y-4">
              <h2 className="text-sm font-semibold text-slate-800">Environments</h2>
              <p className="text-xs text-slate-400">Add the base URLs of your API environments.</p>
              {envs.length > 0 && (
                <div className="divide-y divide-slate-100 rounded-xl border border-slate-200">
                  {envs.map(e => (
                    <div key={e.id} className="flex items-center gap-3 px-4 py-2.5">
                      <p className="text-xs font-medium text-slate-700">{e.name}</p>
                      <p className="flex-1 truncate text-xs font-mono text-slate-400">{e.baseUrl}</p>
              ...
              <div className="flex gap-2">
                <input value={envName} onChange={e => setEnvName(e.target.value)} placeholder="e.g. Staging"
                  className="w-32 rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700 placeholder:text-slate-400 focus:border-indigo-400 focus:outline-none" />
            ...
            <div className="border-t border-slate-100 pt-6 space-y-4">
              ...
              <p className="text-xs text-slate-400">API keys, tokens, and auth credentials for your endpoints. All values are AES-256 encrypted at rest — never stored in plaintext.</p>
              {creds.length > 0 && (
                <div className="divide-y divide-slate-100 rounded-xl border border-slate-200">
                  {creds.map(c => (
                    <div key={c.id} className="flex items-center gap-3 px-4 py-2.5">
                      ...
                      <p className="flex-1 text-xs text-slate-700">{c.name}</p>
                      <p className="text-xs text-slate-400 font-mono">{'•'.repeat(8)}{c.value.slice(-4)}</p>
        ...
        {step === 5 && (
          <div className="space-y-4">
            <h2 className="text-sm font-semibold text-slate-800">Review & Launch</h2>

            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-xl border border-slate-200 p-4">
                <p className="text-xs font-medium uppercase tracking-wider text-slate-400 mb-2">Project</p>
                <p className="text-sm font-semibold text-slate-800">{projectName || 'Untitled'}</p>
                {projectDesc && <p className="mt-1 text-xs text-slate-500">{projectDesc}</p>}
              </div>
              <div className="rounded-xl border border-slate-200 p-4">
                <p className="text-xs font-medium uppercase tracking-wider text-slate-400 mb-2">Inputs</p>
                <p className="text-sm text-slate-700">{specs.length} spec document{specs.length !== 1 ? 's' : ''}</p>
                <p className="text-sm text-slate-700">{repos.length} repositor{repos.length !== 1 ? 'ies' : 'y'}</p>
                <p className="text-sm text-slate-700">{envs.length} environment{envs.length !== 1 ? 's' : ''}</p>
                <p className="text-sm text-slate-700">{creds.length} credential{creds.length !== 1 ? 's' : ''}</p>
              </div>
            </div>
